...
export function UsersCard() {
    ...
    const usersUIProps = useMemo(() => {
        return {
            ...
            openDeleteUserDialog: usersUIContext.openDeleteUsersDialog,
    ...
    const setIsFiltering = () => {
        ...
        if (usersUIProps.isFiltering) {
            const newQueryParams = prepareFilter(usersUIProps.queryParams, {isActive:"true"});
            usersUIContext.setQueryParams(newQueryParams)
        }
    }
    // const { lastError } = useSelector(
    //     (state) => ({ lastError: state.users.lastError }),
    //     shallowEqual
    // );
    return (
        <>
                ...
                <CardHeader title="Kullanıcı Listesi">
                    <CardHeaderToolbar>
                        <button
                            ...
                            <span className="menu-text">{usersUIProps.isFiltering?"Filtreyi Temizle":"Filtrele"}</span>
                        </button>
                        <button
                            type="button"
                            className="btn btn-sm btn-primary mr-1"
